...
public class BOJ_20665_독서실거리두기_나이브_성공 {
	// 입력고정
	...
	public static void main(String[] args) throws Exception {
		...
		reservationList.sort(Comparator
			...
			.thenComparing((reservation) -> reservation.outTime)
		);
		// 세팅
...
//			System.out.println(nowTime+Arrays.toString(seatANDOutTime));
			...
			// 현재 시간 작업 종료
			//// 사용가능 여부 확인
			if(seatANDOutTime[P].equals("빈 좌석")) {
//				System.out.println(nowTime+Arrays.toString(seatANDOutTime));
				useTime++;
			}
			//// 시간이동
	...
	static void assignSeat(String outTime) {
		...
		for (int seatNum = 0; seatNum < N; seatNum++) {
			if(seatANDOutTime[seatNum].equals("빈 좌석")==false) { // 사용중임 => 퇴실 이후에 해야 됨
